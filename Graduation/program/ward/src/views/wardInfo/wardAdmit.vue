<template>
  <div class="content d-flex flex-column align-items-center">
    <div class="card px-3 py-3" style="width: 90%">
      <h2 class="py-3 text-center">住院信息登记</h2>
...
export default {
  ...
  mounted() {
    axios.get("http://localhost:3000/wardInfo/freeBed")
        .then(res => {
          let option = [
            {
              value: 1,
  ...
  methods: {
    submitForm(formName) {
      this.ruleForm.wardId = this.ruleForm.ward[0]
      this.ruleForm.bedId = this.ruleForm.ward[1]
      this.$refs[formName].validate((valid) => {
        if (valid) {
          console.log(this.ruleForm)
          axios.get("http://localhost:3000/patient/addPatient", {
            params: {
              ...this.ruleForm
            }
          }).then((res) => {
